...
const fromDataInnerList = $ => {
  ...
  let k = $(".pagination").eq(1).children().slice(-1).find("a").attr("href").split("/")
  obj.allPage = k[k.length-1].slice(0,-5) //分页数量
  
  $(".pagination").eq(0).children().find("a").each(function(){
    let sortarr = {}
  ...
  $(".pagination").eq(1).children().slice(0).each(function(){
    let pagingarr = {}
  ...
  $('.container div .row').children().each(function (index, element) {
    let rowChild = {}
    rowChild.index = index
    rowChild.link = $(this).find(".movie-name").attr("href")
    rowChild.title = $(this).find(".movie-name").attr("title")
...
module.exports = {
  fromDataHomeList,fromDataInnerList
}
